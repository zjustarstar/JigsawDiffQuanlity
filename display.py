"""this is a module for display image"""
# 5 part
...
def plot_bboxes(image, boxes, labels=[], colors=[], score=True, conf=None):
    ...
    #
    # if IN_COLAB:
    #     cv2_imshow("2",image)  # if used in Colab
    # else:
    #     cv2.imshow("2",image)  # if used in Python
...
def add_patch_line(img,n,linecolor=(0,0,0)):
    w, h, _ = img.shape
    b_h=h//n
    b_w=w//n
    for i in range(1,n):
        ...
        cv2.line(img,start,end,linecolor,2)
...
def add_patch_ID(img, n, text_color=(0,0,255)):
    ...
    for i in range(n):
        for j in range(n):
            ...
            # 在块上写上编号
            cv2.putText(img, str(i * n + j), (start_col + 5, start_row + 60), cv2.FONT_HERSHEY_SIMPLEX, 2,
                        text_color, 4)

    return img

def add_patch_class(img,n,cluster_ids,text_color=(0,0,0)):
    rows, cols, _ = img.shape
    ...
    for i in range(n):
        for j in range(n):
            ...
            cv2.putText(img,str(cluster_ids[i][j]),(start_col + 5, start_row + 60), cv2.FONT_HERSHEY_SIMPLEX, 2,
                        text_color, 4)
